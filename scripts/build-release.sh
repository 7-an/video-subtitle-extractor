...
ARCHIVE_NAME="video-subtitle-extractor-v$VERSION-macos.zip"
...
rm -rf "$PACKAGE_DIR/extension/tests" "$PACKAGE_DIR/native-host/tests" "$PACKAGE_DIR/native-host/__pycache__"
chmod 755 "$PACKAGE_DIR/install-macos.command" "$PACKAGE_DIR/uninstall-macos.command"

rm -f "$DIST_DIR/$ARCHIVE_NAME" "$DIST_DIR/SHA256SUMS.txt"
cd "$STAGE_DIR"
zip -qr "$DIST_DIR/$ARCHIVE_NAME" "$(basename "$PACKAGE_DIR")"
cd "$DIST_DIR"
