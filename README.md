...
1. 下载并解压最新版安装包。
2. 直接双击新版 `install-macos.command`，它会覆盖程序文件并保留已下载的模型。
3. 打开 `chrome://extensions`，在“视频字幕提取插件”卡片上点击“重新加载”。

不需要先删除旧扩展，也不要先运行卸载脚本；卸载脚本会同时删除本地助手和模型缓存。

扩展 ID 固定为：`nhhnpcghjppnpenldjjaofbnfkpoogdf`。

首次使用某个本地识别模型时会单独下载，模型会缓存在：
...
4. 如果没有字幕轨，选择 Whisper 模型和语音语言，再点击“本地生成字幕”。
5. 识别完成后选择 MD、SRT、TXT 或 JSON 下载。

本地模型可选：

- `tiny`：最快，适合快速初稿，准确度最低。
- `base`：速度较快，准确度略高于 `tiny`。
- `small`：默认推荐，兼顾速度、内存和准确度。
- `medium`：准确度优先，但速度更慢、占用更多内存。

模型和语音语言的选择会自动保存。指定正确的语音语言通常比“自动检测”更稳定；这些选项只影响本地语音识别，不影响直接提取 YouTube 已有字幕。
